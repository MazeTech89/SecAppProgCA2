// Backend API tests (secure suite)
// Note: on the insecure branch, secure-only routes (e.g., /csrf-token) are not present,
// so this suite auto-skips to avoid false failures.
...
function hasRoute(expressApp, method, path) {
  const stack = expressApp && expressApp._router && Array.isArray(expressApp._router.stack)
  ...
  return stack.some((layer) => {
    ...
    return Boolean(layer.route.methods && layer.route.methods[m]);
  });
}

// This test suite targets the "secure" backend implementation.
// If the current branch/code does not expose the secure routes (e.g., /csrf-token), skip the suite.
const describeSecure = hasRoute(app, 'get', '/csrf-token') ? describe : describe.skip;
...
describeSecure('SecureApp API (use-case derived)', () => {
  afterAll((done) => {
